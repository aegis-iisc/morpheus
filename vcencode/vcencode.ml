...
(*FIXME :: Add support for automatically find qualifiers
Add support for \forall formulas *)
open SpecLang 
open Z3_encode
          ...
          (*cdecl sorts *)    
           ...
           let  (tyMap, constMap, relMap, sort_externfun) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.fromString "externfun") in 
             ...
             let  (tyMap, constMap, relMap, sort_ulist) = 
            encodeTyD (tyMap, constMap, relMap) (Ty_list (TyD.Ty_unit)) in 


          (* includes predicate *)
          (* let  (tyMap, constMap, relMap, sort_a) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.fromString "'a") in 


          let  (tyMap, constMap, relMap, sort_result) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.Ty_param (TyD.fromString "'a", TyD.fromString "result")) in 
 *)
          let  (tyMap, constMap, relMap, sort_result) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.fromString "result") in 

          let  (tyMap, constMap, relMap, sort_err) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.fromString "err") in 
          
          let inr = mkStrucRel ("Inr", 
                        [sort_err; sort_result]) in 
          let relMap = RelMap.add relMap ("Inr") inr in 
    

          let  (tyMap, constMap, relMap, sort_charresult) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.fromString "char_result") in 

            let  (tyMap, constMap, relMap, sort_charres_list) = 
            encodeTyD (tyMap, constMap, relMap) (Ty_list (TyD.fromString "char_result")) in 
 
           let  (tyMap, constMap, relMap, sort_charerr) = 
            encodeTyD (tyMap, constMap, relMap) (TyD.fromString "char_err") in 
          

          let includes = mkMStrucRel ("includes", 
                        [sort_heap; sort_heap; sort_char_list; sort_bool]) in 
          let relMap = RelMap.add relMap ("includes") includes in 

          (* for now it has a type integer -> result *)
          let inlchar = mkStrucRel ("Inlchar", 
                        [sort_string; sort_charresult]) in 
          let relMap = RelMap.add relMap ("Inlchar") inlchar in 
    
           let inrchar = mkStrucRel ("Inrchar", 
                        [sort_charerr; sort_charresult]) in 
          let relMap = RelMap.add relMap ("Inrchar") inrchar in 
    
          let crlen = mkStrucRel ("crlen", 
                        [sort_charres_list; sort_int]) in 
          let relMap = RelMap.add relMap ("crlen") crlen in 
         ...
         let relMap = RelMap.add relMap ("isize") isize in 
          ...
          let ulen = mkStrucRel ("ulen", [sort_ulist;sort_int]) in 
          let relMap = RelMap.add relMap ("ulen") len in 
        ...
        let
          ...
          let encodeBaseExpr bexp = 
            ...
            | Var v -> 
              ...
              getConstForVar constMap v
            | Char c -> 
              let () = Printf.printf "%s " ("\n $$$$$$$$$$$$$$$$$-encdode BaseExp Char "^(Char.escaped c)) in 
              let c_const = mkStr (Char.escaped c) in 
              c_const
            | Str s ->
