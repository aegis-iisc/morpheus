...
let sort_layout = fun s ->
          ...
          | Char c -> Layout.str (Z3Sort.to_string c)
          | Str s -> Layout.str (Z3Sort.to_string s)
...
let typeCheckAst (AST (ast,sort),sort') = 
  ...
  | (Char _, Char _) -> true 
  | (_,_) -> 
    Printf.printf "%s" ("Sort mismatch: other , other  "^(sortToString sort)
                              ^" vs "^(sortToString sort')); 
    false 
