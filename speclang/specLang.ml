...
(*check if two types are same*)
  let rec sametype t1 t2 = 
        ...
        | (Ty_param (t11, t12), Ty_param (t21, t22)) -> (sametype t11 t21) && (sametype t12 t22)
        | (Ty_int, Ty_int) -> true
...
(*base types from string*)    
 ...
 let rec toString t = 
      match t with
        ...
        | Ty_param (t1, t2) -> (toString t1^" "^toString t2)        
        | Ty_int -> "Ty_int"
        | Ty_bool -> "Ty_bool"
        | Ty_char -> "Ty_char"        
        | Ty_heap  -> "Ty_heap"
...
struct
  ...
  module BasePredicate = struct
   ...
   let exprToString e = 
    ...
    let varCharEq (v1, c) = Eq (Var v1, Char c )
...
let rec getBVs t = 
        match t with 
        | Forall (binds, t1) -> 
                List.concat[binds;(getBVs t1)]
        
        | Conj (e1,e2) -> List.append (getBVs e1) (getBVs e2) 
        | Disj (e1,e2) ->  List.append (getBVs e1) (getBVs e2) 
        | If (e1,e2) -> List.append (getBVs e1) (getBVs e2) 
        | Iff (e1,e2) -> List.append (getBVs e1) (getBVs e2) 
        | Dot (e1,e2) ->  List.append (getBVs e1) (getBVs e2) 
        | _ -> [] 
...
let rec toString t = match t with
      ...
      | Conj (e1,e2) -> ("\n \t  BEGIN "^(toString e1 )^" &&  "^(toString e2)^" END ")
  ...
  let rec applySubst ((nw, ol) as subst) t = 
    ...
    | Forall (tyDB,tint) -> 
        ...
        *) (* if (TyDBinds.mem tyDB ol)
        ...
        Forall (tyDB,applySubst subst tint)
   
 (*reduce  
 we do not have \x. p x in our spec language 
 This is taken care by this reduction funtion
 ...
 let rec reduce ((nw, ol) as subst) t = 
    ...
    | Dot (t1,t2) -> Dot (applySubst subst t1, applySubst subst t2)
    | Forall (tyDB,t1) ->
         (* let () = Printf.printf "%s"  ("\n reducing predicate "^(toString t)) in
         let () = Printf.printf "%s" ("\n New "^(nw)^(" Old "^(ol))) in *)
         (applySubst subst  t1)
  ...
  let reduceId v t = 
      reduce (v, v) t 
  (* Applying a muti-variable functions *)
  let rec reduceMany substs t = 
    List.fold_right (fun subst t ->
        reduce subst t) substs t 

  let reduceManyId v t =
     reduceMany (List.combine v v) t 
...
(*   
  ...
  let rec applySubsts substs refty = 
	    match refty with 
          | Base (v,t,p) -> 
            Base (v,t,Predicate.applySubsts substs p)
          
          | Arrow ((v1,t1),t2) -> Arrow ((v1, applySubsts substs t1), 
                                            applySubsts substs t2)
          | MArrow (eff, pre, (v, t), post) -> 
                MArrow (eff, (Predicate.applySubsts substs pre), 
                              (v, (applySubsts  substs t)), 
                              (Predicate.applySubsts substs post)
                      )
          | Sigma ls ->  Sigma (List.map (fun (v, t) -> (v, (applySubsts substs t))) ls)
          
          | Uncurried (arg_type_list, t2) -> 
            Uncurried (
                      List.map (fun (vi, tyi) -> 
                                  (vi, applySubsts substs tyi)) arg_type_list,
                      applySubsts substs t2
                      )
          | _ -> raise (RefTyEx "Unhandled Substitution")



  let  alphaRenameToVar refty newbv =
     match refty with
