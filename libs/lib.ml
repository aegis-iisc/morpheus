...
let ( <* ) p1 e2 = Eparser (Bind (p1, Elam ([Evar "_i"], 
                                Eparser (Bind (e2, Eret (Evar "_i"))))))



let assign v e2 eb = e2 >>= (Elam ([Evar v], eb))  
let apply funName args = Eapp (Evar funName, List.map (fun aiName -> Evar aiName) args)
let dostmt ls lastExp = 
    let rec term (altacc : monExp) 
                remaining = 
    ...
    (term lastExp (List.tl (List.rev ls)))       
...
let letter = any [Char 'a'; Char 'b'; Char 'c';Char 'z']
...
let many p = star p
let identifier = many (Var "letter")
