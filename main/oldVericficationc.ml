...
let () = Printf.printf "%s" ("\n >>>>>>>>>>>Inferred Type<<<<<<<<<<<<< "^(RefTy.toString inferred)) in 
  match (inferred , annotated) with
       ...
       | (MArrow (eff1, p1 , (v1, t1) , p1'), MArrow (eff2, p2, (v2, t2), p2')) -> 
              ...
              let p1_applied  = Predicate.reduceManyId (List.map (fun (vi, ti) -> vi) bvs_p1) p1 in  
...
(*                 let () = Printf.printf "%s" ("\n Original Annotatetd pre "^P.toString p2) in  *)
              ...
              (* let phi_annot_imp_pre = P.If (p2_temp, p1_temp) in  *)
                ...
                let phi_annot_imp_pre = P.If (p2_applied, p1) in  
                ...
                (* let () = Printf.printf "%s" ("\n After App p2' "^(P.toString p1'_temp)) in  *)
              ...
              let p1'_reduced = Predicate.reduceManyId (List.map (fun (vi, ti) -> vi) bvs_p1') p1' in 
             
              let () = Printf.printf "%s" ("\n After New App p1' "^(P.toString p1'_reduced)) in  
             

              let () = Printf.printf "%s" ("\n $$$$$$$$$$$$ p2' "^(P.toString p2')) in 
             
              let p2'_reduced = Predicate.reduceMany subs p2' in 
  
             let () = Printf.printf "%s" ("\n After New App p2' "^(P.toString p2'_reduced)) in  
              ...
              *)
              ...
              let ante_conj = P.True in 
                ...
                let phi_post_imp_anno = P.If (p1'_reduced, p2'_reduced)  in  
